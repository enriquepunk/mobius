...
/**
* @version		Content 2.1: jsocial_comments.php
* @copyright	Copyright (C) 2011 PloPixel. All rights reserved.
...
// Plugin JSocial Comments
class plgContentJSocial_Comments extends JPlugin
{

	public function __construct(& $subject, $config)
	{
		parent::__construct($subject, $config);
		$plugin = JPluginHelper::getPlugin('system', 'jsocial_comments');
		jimport ('joomla.html.parameter');
		$this->system = new JParameter($plugin->params);
		$this->loadLanguage();
	}
	...
	public function onContentAfterDisplay($context, &$row, &$params, $limitstart=0)
	{
		...
		if(JPluginHelper::isEnabled('system','jsocial_comments') == false) return;
		...
		// tools
		$tools = new jsocialComments();

		// Site lang
		$site_lang = ($this->system->get('auto_language', 1)) ? JFactory::getLanguage()->getTag() : $this->system->get('language_tag', 'en_GB');

		// Filter
		$is_featured = $tools->isFeatured();
		$is_blog = $tools->isBlog();
		$is_article = $tools->isArticle();
		$bar_featured = $this->params->get('bar_featured', 1);
		$bar_blog = $this->params->get('bar_blog', 1);
		if ((!$is_featured || !$bar_featured) && (!$is_blog || !$bar_blog) && !$is_article) return '';

		// Get document
		$document =& JFactory::getDocument();

		// Site url
		$site_url = JURI::getInstance()->toString(array('scheme', 'host', 'port')) . JRoute::_(ContentHelperRoute::getArticleRoute($row->slug, $row->catid));

		// Article text
		$article_text = isset($row->text) ? $row->text : $row->introtext;

		// Article image
		$image = $this->system->get('default_image', 0) ? $tools->grabImage($this->system->get('article_image', '')) : $tools->grabImage($this->system->get('article_image', ''), $article_text);
		...
		if ($is_article) {
			// facebook meta
			$text = $row->metadesc ? $tools->cleanText($row->metadesc.' | '.$row->introtext) : $tools->cleanText($row->introtext);
			$desc = mb_strlen($text) > 300 ? utf8_substr($text, 0, 297).'...' : $text;
			$document->setMetaData('fbtag_og:author', $row->author);
			$document->setMetaData('fbtag_og:title', $row->title);
			$document->setMetaData('fbtag_og:description', $desc);
			$document->setMetaData('fbtag_og:url', $site_url);
			$document->setMetaData('fbtag_og:image', JURI::root().$image);
		}

		$allowed_cat = $this->params->get('catid');
		if (!is_array($allowed_cat) || !in_array($row->catid, $allowed_cat)) return;
		...
		// Color scheme
		$color_scheme = $this->system->get('color_scheme', 0);

		// comments on ?
		$comments = $this->params->get('comments', 1);

		// Share buttons & Comments count
		$bar_article = $this->params->get('bar_article', 1);
		if(($is_featured && $bar_featured) || ($is_blog && $bar_blog) || ($is_article && $bar_article)) {

			$sp_f = $this->params->get('show_plusone_f', 1);
			$sp_b = $this->params->get('show_plusone_b', 1);
			$sp_a = $this->params->get('show_plusone_a', 1);
			$sl_f = $this->params->get('show_linkedin_f', 1);
			$sl_b = $this->params->get('show_linkedin_b', 1);
			$sl_a = $this->params->get('show_linkedin_a', 1);
			$sb_f = $this->params->get('show_buzz_f', 1);
			$sb_b = $this->params->get('show_buzz_b', 1);
			$sb_a = $this->params->get('show_buzz_a', 1);
			$st_f = $this->params->get('show_twitter_f', 1);
			$st_b = $this->params->get('show_twitter_b', 1);
			$st_a = $this->params->get('show_twitter_a', 1);
			$sf_f = $this->params->get('show_facebook_f', 1);
			$sf_b = $this->params->get('show_facebook_b', 1);
			$sf_a = $this->params->get('show_facebook_a', 1);
			$sc_f = $this->params->get('show_count_f', 0);
			$sc_b = $this->params->get('show_count_b', 0);
			$sc_a = $this->params->get('show_count_a', 0);
			$bl_f = $this->params->get('button_layout_f', 6);
			$bl_b = $this->params->get('button_layout_b', 4);
			$bl_a = $this->params->get('button_layout_a', 1);

			if ($is_featured) {
				$buttons = $tools->grabButtons($bl_f);
			} else if ($is_blog) {
				$buttons = $tools->grabButtons($bl_b);
			} else {
				$buttons = $tools->grabButtons($bl_a);
			}

			// Facebook button
			$fb_position = '';
			if (($is_featured && $sf_f) || ($is_blog && $sf_b) || ($is_article && $sf_a)) {
				$facebook_send = $this->params->get('facebook_send', 'true');
				$facebook_faces = $this->params->get('facebook_faces', 'false');
				$facebook_verb = ($this->params->get('facebook_verb', '')) ? 'action="'.$this->params->get('facebook_verb', '').'" ' : '';
				$facebook_font = ($this->params->get('facebook_font', '')) ? $this->params->get('facebook_font', '') : '';
				if ($buttons->number == 5 || $buttons->number == 6) {
					$facebook_width = 70;
					$fb_position = 'first';
				} else {
					$facebook_width = $this->params->get('facebook_width', 450);
					$fb_position = 'last';
				}

				$fb_html = '<span class="jsc-facebook"><fb:like href="'.$site_url.'" send="'.$facebook_send.'" width="'.$facebook_width.'" show_faces="'.$facebook_faces.'" '.($buttons->facebook != 'none' ? 'layout="'.$buttons->facebook.'" ' : '').$facebook_verb.($color_scheme ? 'colorscheme="dark" ' : '').'font="'.$facebook_font.'"></fb:like></span>';
			}

			// Start share bar
			$share_html .= '<div class="jsc-bar '.$buttons->style.'">';

			// Show Facebook button (First)
			if ($fb_position == 'first') $share_html .= $fb_html;

			// Plus One button
			if (($is_featured && $sp_f) || ($is_blog && $sp_b) || ($is_article && $sp_a)) {
				if (!defined('_SYS_JSC_PO')) {
					define ('_SYS_JSC_PO',1);
					$plusone_lang = $tools->grabLanguage($site_lang, 'plusone');
					$plusone_js_lang = $plusone_lang ? "window.___gcfg={lang:'$plusone_lang'};" : '';
					$document->addCustomTag("<script type=\"text/javascript\">".$plusone_js_lang."LazyLoad.js('http://apis.google.com/js/plusone.js', function(){});</script>");
				}

				// Show Plus One button
				$share_html .= '<span class="jsc-plusone"><g:plusone size="'.$buttons->plusone.'" href="'.$site_url.'"></g:plusone></span>';
			}

			// Buzz button
			if (($is_featured && $sb_f) || ($is_blog && $sb_b) || ($is_article && $sb_a)) {
				if (!defined('_SYS_JSC_BZ')) {
					define ('_SYS_JSC_BZ',1);
					$document->addCustomTag("<script type=\"text/javascript\">LazyLoad.js('http://www.google.com/buzz/api/button.js', function(){});</script>");
				}
				$buzz_lang = $tools->grabLanguage($site_lang, 'buzz');
				$buzz_message = ($row->metadesc) ? ' data-message="'.JText::sprintf('PLG_CONTENT_JSOCIAL_COMMENTS_BUZZ_MESSAGE', $app->getCfg('sitename'), $row->metadesc).'"' : '';

				// Show Buzz button
				$share_html .= '<span class="jsc-buzz"><a rel="nofollow" title="Google Buzz" class="google-buzz-button" href="http://www.google.com/buzz/post" data-button-style="'.$buttons->buzz.'" data-locale="'.$buzz_lang.'" data-url="'.$site_url.'" data-imageurl="'.JURI::root().$image.'"'.$buzz_message.'></a></span>';
			}

			// Linkedin button
			if (($is_featured && $sl_f) || ($is_blog && $sl_b) || ($is_article && $sl_a)) {

				if (!defined('_SYS_JSC_LI')) {
					define ('_SYS_JSC_LI',1);
					$document->addCustomTag("<script type=\"text/javascript\">LazyLoad.js('http://platform.linkedin.com/in.js', function(){});</script>");
				}

				// Show Linkedin button
				$share_html .= '<span class="jsc-linkedin"><script type="IN/Share" data-counter="'.$buttons->linkedin.'" data-url="'.$site_url.'"></script></span>';
			}

			// Twitter button
			if (($is_featured && $st_f) || ($is_blog && $st_b) || ($is_article && $st_a)) {

				if (!defined('_SYS_JSC_TW')) {
					define ('_SYS_JSC_TW',1);
					$document->addCustomTag("<script type=\"text/javascript\">LazyLoad.js('http://platform.twitter.com/widgets.js', function(){});</script>");
				}

				$twitter_lang = $tools->grabLanguage($site_lang, 'twitter');
				$twitter_via = ($this->params->get('twitter_via', '')) ? 'data-via="'.$this->params->get('twitter_via', '').'" ' : '';
				$twitter_related_desc = ($this->params->get('twitter_related_desc', '')) ? ':'.$this->params->get('twitter_related_desc', '') : '';
				$twitter_related = ($this->params->get('twitter_related', '')) ? 'data-related="'.$this->params->get('twitter_related', '').$twitter_related_desc.'" ' : '';

				// Show button
				$share_html .= '<span class="jsc-twitter"><a rel="nofollow" href="http://twitter.com/share" class="twitter-share-button" data-url="'.$site_url.'" data-text="'.$row->title.'" data-count="'.$buttons->twitter.'" '.$twitter_via.$twitter_related.'data-lang="'.$twitter_lang.'">Tweet</a></span>';
			}

			// Show Facebook button (Last)
			if ($fb_position == 'last') $share_html .= $fb_html;

			// Comments count
			if ($comments && (($is_featured && $sc_f) || ($is_blog && $sc_b) || ($is_article && $sc_a))) {
				if ($buttons->type == 'big') {
					$share_html .= '<span class="jsc-count-big">
					<span class="jsc-big">
					<span class="jsc-big-nub"></span>
					<span class="jsc-big-num">
					<span class="jsc-big-log">
					<fb:comments-count href='.$site_url.'></fb:comments-count>
					</span>
					</span>
					<a class="jsc-big-but" href="'.$site_url.'#comments" title="'.JText::_('PLG_CONTENT_JSOCIAL_COMMENTS_COMMENTS_ADD').'">
					<span class="jsc-big-label">
					<span class="jsc-big-text">'.JText::_('PLG_CONTENT_JSOCIAL_COMMENTS_COMMENTS').'</span>
					</span>
					</a>
					</span>
					</span>';
				} else {
					$share_html .= '<span class="jsc-count-small">
					<a class="jsc-small-but" href="'.$site_url.'#comments" title="'.JText::_('PLG_CONTENT_JSOCIAL_COMMENTS_COMMENTS_ADD').'">
					<span class="jsc-small-label">
					<span class="jsc-small-text">
					<span class="jsc-small-fb">
					<fb:comments-count href='.$site_url.'></fb:comments-count>
					</span>
					<span class="jsc-small-desc">'.JText::_('PLG_CONTENT_JSOCIAL_COMMENTS_COMMENTS').'</span>
					</span>
					</span>
					</a>
					</span>';
			...
			$share_html .= '<div class="clear"></div></div>';
		}

		// Facebook comments
		if ($comments && $is_article) {
			$device = $tools->detectDevice();
			if ($this->params->get('detect_mobile', 1) && ($device == 'iphone' || $device == 'general')) {
				$comment_width = ($device == 'iphone') ? $this->params->get('device_iphone', 500) : $this->params->get('device_general', 400);
			} else {
				$comment_width = $this->params->get('comment_width', 500);
			}
			$comment_html .= '<a name="comments" id="comments"></a><div class="jsc"><fb:comments href="'.$site_url.'" num_posts="'.$this->system->get('post_count', 5).'" width="'.$comment_width.'"'.($color_scheme ? ' colorscheme="dark"' : '').'></fb:comments></div>';
			$show_feeds = $this->params->get('show_feeds', 2);
			if ($show_feeds) {
				$feed = $tools->getComments($site_url, 'facebook', $this->system->get('comment_cache', 600));
				if ($feed) {
					if ($show_feeds == 2) {
						$comment_html .= '<div class="jsc-feeds"><span>'.JText::_('PLG_CONTENT_JSOCIAL_COMMENTS_ROBOTS_MESSAGE').'</span>'.$feed.($this->params->get('show_consid', 1) ? base64_decode('IHwgSlNvY2lhbCBDb21tZW50cyBieSA8YSBocmVmPSJodHRwOi8vd3d3LnBsb3BpeGVsLmNvbSIgdGl0bGU9IkpTb2NpYWwgQ29tbWVudHMgYnkgUGxvUGl4ZWwiPlBsb1BpeGVsPC9hPg==') : '').'</div>';
					} else {
						$comment_html .= '<noscript><p>'.JText::_('PLG_CONTENT_JSOCIAL_COMMENTS_ROBOTS_MESSAGE').' '.$feed.($this->params->get('show_consid', 1) ? base64_decode('IHwgSlNvY2lhbCBDb21tZW50cyBieSA8YSBocmVmPSJodHRwOi8vd3d3LnBsb3BpeGVsLmNvbSIgdGl0bGU9IkpTb2NpYWwgQ29tbWVudHMgYnkgUGxvUGl4ZWwiPlBsb1BpeGVsPC9hPg==') : '').'</p></noscript>';
					}
				}
			}
		}
		...
		// Switch style
		switch($buttons->number) {
			case 1:
			case 3:
			case 7:
			$article_text = $article_text.$share_html.$comment_html;
			break;
			default:
			$article_text = $share_html.$article_text.$comment_html;
			break;
		}
