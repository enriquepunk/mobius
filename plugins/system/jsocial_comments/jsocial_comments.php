...
/**
* @version		System 2.1: jsocial_comments.php
...
include_once(JPATH_ROOT.DS.'plugins'.DS.'system'.DS.'jsocial_comments'.DS.'libs'.DS.'jsocial_tools.php');

// Plugin JSocial Comments
class plgSystemJSocial_Comments extends JPlugin
{
	...
	function onAfterDispatch()
	{
		...
		if (!$app->isSite() || $app->isAdmin() || JFactory::getDocument()->getType() !== 'html') return;

		// document
		$document =& JFactory::getDocument();

		// lazyload
		$document->addScript(JURI::root().'plugins/system/jsocial_comments/assets/js/lazyload.js');

		// tools
		$tools = new jsocialComments();

		// Site lang
		$site_lang = ($this->params->get('auto_language', 1)) ? JFactory::getLanguage()->getTag() : $this->params->get('language_tag', 'en_GB');

		// css
		$plg_ver = '2.0';
		if ($this->params->get('color_scheme', 0)) {
			$css = 'plugins/system/jsocial_comments/assets/css/dark.css?ver='.$plg_ver;
		} else {
			$css = 'plugins/system/jsocial_comments/assets/css/light.css?ver='.$plg_ver;
		}
		$document->addStyleSheet(JURI::root() . $css);

		// facebook js
		$lang_code = $tools->grabLanguage($site_lang, 'facebook');
		if (!defined('_SYS_JSC_FB')) {
			define ('_SYS_JSC_FB',1);
			$document->addCustomTag("<script type=\"text/javascript\">LazyLoad.js('http://connect.fbtag_facebook.net/$lang_code/all.js#xfbml=1', function(){});</script>");
		}

		// facebook meta
		$fb_userid = $this->params->get('fb_userid');
		$fb_appid = $this->params->get('fb_appid');
		$site_name = $app->getCfg('sitename');

		if (!$tools->isArticle()) {
			$document->setMetaData('fbtag_og:author', $this->params->get('alt_author', 'Administrator'));
			//$document->setMetaData('fbtag_og:author', 'Cesaringo3');
                        
                        $document->setMetaData('fbtag_og:title', htmlspecialchars($document->getTitle(), ENT_COMPAT, 'UTF-8'));
			$desc = $document->getDescription();
			if (!$desc) $desc = $site_name;
			$document->setMetaData('fbtag_og:description', htmlspecialchars($desc));
			$document->setMetaData('fbtag_og:url', JURI::getInstance()->toString());
			$document->setMetaData('fbtag_og:image', JURI::root().$tools->grabImage($this->params->get('article_image', '')));
		}
                
                
		if ($fb_userid) $document->setMetaData('fbtag_fb:admins', $fb_userid);
		if ($fb_appid) $document->setMetaData('fbtag_fb:app_id', $fb_appid);
		$document->setMetaData('fbtag_og:language', str_replace('_', '-', $lang_code));
		$document->setMetaData('fbtag_og:site_name', $site_name);
		$document->setMetaData('fbtag_og:type', $this->params->get('object_type', 'article'));
	}
	...
	public function onAfterRender()
	{
		...
		// debug
		$debug_act = $this->params->get('show_debug', 0);
		if ($debug_act) $message = '';

		// Fix for W3C Validator
		$validator = $this->params->get('w3c_validator', 1);
		if ($validator || $debug_act) {
			preg_match_all('/xmlns:([fb|og]{2})=/i', $html, $matches);
			$fb_tag = 'xmlns:fb="http://developers.facebook.com/schema/"';
			$og_tag = 'xmlns:og="http://ogp.me/ns#"';
			if (!isset($matches[1][0])) {
				$html = str_replace('<html ', '<html '.$og_tag.' '.$fb_tag.' ', $html);
				if ($debug_act && !$validator) $message .= '<span class="jsc-info">- INFORMATION (W3C Tag): (xmlns:fb) and (xmlns:og) inserted inside the "HTML" tag!</span>';
			} else if (count($matches[1]) == 1) {
				$ins_tag = strtolower($matches[1][0]) == 'fb' ? $og_tag : $fb_tag;
				$html = str_replace('<html ', '<html '.$ins_tag.' ', $html);
				if ($debug_act && !$validator) $message .= '<span class="jsc-info">- INFORMATION (W3C Tag): (xmlns:'.$matches[1][0].') inserted inside the "HTML" tag!</span>';
			}
			unset($matches);
		}
		...
		// Facebook .js
		$rep_tag = true;
		$fix_fb = $this->params->get('facebook_js', 1);
		if ($fix_fb || $debug_act) {
			preg_match_all('/connect\.facebook\.net\/([a-zA-Z_]+)\//i', $html, $matches);
			if (isset($matches[0][0])) {
				$html = preg_replace('/<script.*src=["\']?http:\/\/connect\.facebook\.net\/([a-zA-Z_]*)\/all\.js.*?["\']?.*>.*<\/script>/i', '', $html, -1 , $count);
				if (count($matches[0]) != $count) {
					// could not remove all the javascript :S so... we fix the facebook Lang?
					$lang_code = $tools->grabLanguage($site_lang, 'facebook');
					$fb_ori = $matches[1][0];
					if ($this->params->get('correct_language', 1) && $fb_ori != $lang_code) {
						$html = str_replace('connect.facebook.net/'.$fb_ori.'/', 'connect.facebook.net/'.$lang_code.'/', $html);
						if ($debug_act && !$fix_fb) $message .= '<span class="jsc-info">- INFORMATION (Facebook Lang Fix): Facebook javascript language fixed!</span>';
					}
					// Clean
					$html = str_replace("<script type=\"text/javascript\">LazyLoad.js('http://connect.fbtag_facebook.net/$lang_code/all.js#xfbml=1', function(){});</script>", '', $html);
					if ($debug_act && !$fix_fb) $message .= '<span class="jsc-warn">- WARNING (Facebook JS): You need to activate the option <strong>"Fix Facebook JS"</strong> because there are other facebook javascript in the document!</span>';
					$rep_tag = false;
				}
			}
			unset($matches);
		}
		if ($rep_tag) $html = str_replace('connect.fbtag_facebook.net/', 'connect.facebook.net/', $html);

		// Facebook div tag
		$fix_fdiv = $this->params->get('facebook_div', 1);
		if ($fix_fdiv || $debug_act) {
			preg_match('/<div.*id=["\']?fb-root["\']?.*><\/div>/i',$html ,$matches);
			if (isset($matches[0])) {
				$html = preg_replace('/<div.*id=["\']?fb-root["\']?.*><\/div>/i', '', $html);
				if ($debug_act && !$fix_fdiv) $message .= '<span class="jsc-warn">- WARNING (Facebook DIV): You need to activate the option <strong>"Fix Facebook DIV"</strong> because there are other facebook "fb-root" labels in the document!</span>';
			}
			unset($matches);
		}
		...
		// Facebook og
		$fix_meta = $this->params->get('check_meta', 0);
		if ($fix_meta || $debug_act) {
			preg_match_all('/<meta.*property=["\']([a-zA-Z_:]+)["\'].*\/>/i', $html, $matches);
			if (isset($matches[1][0])) {
				$fb_meta = array('og:author','og:title','og:description','og:url','og:image','fb:admins','fb:app_id','og:language','og:site_name','og:type');
				$intersect = array_intersect($fb_meta, $matches[1]);
				if (count($intersect) > 0) {
					foreach ($intersect as $value) {
						$html = preg_replace('/<meta.*property=["\']'.$value.'["\'].*\/>/i', '', $html);
					}
				}
				if ($debug_act && !$fix_meta) $message .= '<span class="jsc-warn">- WARNING (Facebook Meta): You need to activate the option <strong>"Fix Facebook Meta"</strong> because there are other facebook meta tags in the document!</span>';
			}
			unset($matches);
		}
		$html = str_replace('name="fbtag_', 'property="', $html);

		// Show Debug
		if ($debug_act) {
			if ($message) {
				$message .= '<span class="jsc-mess"><strong>IMPORTANT:</strong> If you see any <strong>"WARNING"</strong> message, activate the recommended option in the <strong>"System - JSocial Comments"</strong> plugin because there are conflicts that can prevent the proper operation of the plugin. <strong>"INFORMATION"</strong> messages are not problems!<br/>';
				$message .= 'Please visit <a href="http://www.plopixel.com/" title="PloPixel">www.plopixel.com</a> to see if you can find a solution to your problems with the plugin, if you have any of course.</span>';
			} else {
				$message .= '<span class="jsc-info" style="padding: 5px;">The debug system has not found any problems. Please disable the debug system in the <strong>"System - JSocial Comments"</strong> plugin.</span>';
			}
			$html = preg_replace("/<body(.*?)>/i", "<body$1>\n<div class=\"jsc-debug\"><div class=\"jsc-title\">JSocial Comments Debug</div>$message</div>", $html);
		}
