...
class jsocialComments
{
	protected $_url = '';
	protected $_service = '';
	protected $_cache = 600;

	public function isFeatured() {
		return (JRequest::getVar('option') == 'com_content' && JRequest::getVar('view') == 'featured') ? true : false;
	}

	public function isBlog() {
		return (JRequest::getVar('option') == 'com_content' && JRequest::getVar('view') == 'category' && JRequest::getVar('layout') == 'blog') ? true : false;
	}

	public function isArticle() {
		return (JRequest::getVar('option') == 'com_content' && JRequest::getVar('view') == 'article') ? true : false;
	}

	public function getComments($url, $service, $cache) {
		$this->_url = $url;
	...
	public function grabImage($image, $text = null) {
		if ($text) {
		...
		if (!file_exists($image)) {
			$image = 'plugins/system/jsocial_comments/assets/images/no_image.jpg';
		}
		return $image;
	}

	public function detectDevice() {
		include_once(JPATH_ROOT.DS.'plugins'.DS.'system'.DS.'jsocial_comments'.DS.'libs'.DS.'detect_browser.php');
		$browser = new detectBrowser();
		if ($browser->isRobot()) return 'robot';
		if ($browser->getBrowser() == detectBrowser::BROWSER_IPAD) return false;
		if ($browser->isMobile()) {
			if (in_array($browser->getBrowser(), array(detectBrowser::BROWSER_IPHONE, detectBrowser::BROWSER_IPOD))) {
				$device = 'iphone';
			} else {
				$device = 'general';
			}
	...
	public function cleanText($text) {
		...
		$text = preg_replace("/<a[^>]*>(.*)<\/a>/iU", " $1 ", $text);
		$text = preg_replace("/<small[^>]*>(.*)<\/small>/iU", '', $text);
		$text = preg_replace('/(([_A-Za-z0-9-]+)(\\.[_A-Za-z0-9-]+)*@([A-Za-z0-9-]+)(\\.[A-Za-z0-9-]+)*)/iex', '', $text);
		$text = preg_replace('(\{.*?\})', ' ', $text);
	...
	public function grabLanguage($site_lang, $service) {
		$lang_code = '';
		$short_lang = substr($site_lang, 0, 2);
		$mod_lang = str_replace('-', '_', $site_lang);
		switch ($service) {
			case 'twitter':
			$langs = array('fr','de','it','ja','ko','ru','es','tr');
			if (in_array($short_lang, $langs)) {
				$lang_code = $short_lang;
			}
			break;
			case 'buzz':
			$langs = array('de','de_CH','gsw','ar','bn','bg','kn','ca','cs','zh','zh_CN','zh_HK','zh_TW','ko','hr','da','sk','sl','es','fil','fi','fr','el','gu','iw','hi','hu','in','en_IN','en_IE','en_GB','en_SG','en_ZA','it','ja','lv','ln','lt','ml','ms','mr','nl','no','or','fa','pl','pt_BR','pt_PT','ro','ru','sr','sv','th','ta','te','tr','uk','vi');
			if (in_array($mod_lang, $langs)) {
				$lang_code = $mod_lang;
			} else if (in_array($short_lang, $langs)) {
				$lang_code = $short_lang;
			}
			break;
			case 'plusone':
			$langs = array('ar','bg','ca','zh-CN','zh-TW','hr','cs','da','nl','en-GB','et','fil','fi','fr','de','el','iw','hi','hu','id','it','ja','ko','lv','lt','ms','no','fa','pl','pt-BR','pt-PT','ro','ru','sr','sv','sk','sl','es','es-419','th','tr','uk','vi');
			if (in_array($site_lang, $langs)) {
				$lang_code = $site_lang;
			} else if (in_array($short_lang, $langs)) {
				$lang_code = $short_lang;
			}
	...
	public function grabButtons($button) {
		...
		if ($button >= 5) {
			...
			$buttons->plusone = 'tall';
			$buttons->buzz = 'normal-count';
			$buttons->twitter = 'vertical';
			$buttons->facebook = 'box_count';
			$buttons->linkedin = 'top';
			$buttons->type = 'big';
		} else if ($button <= 2) {
			// Small + Lange Facebook
			switch ($button) {
				case 1:
				$style = 'small-b large-fb';
				break;
				case 2:
				$style = 'small-t large-fb';
				break;
			}
			$buttons->plusone = 'medium';
			$buttons->buzz = 'small-count';
			$buttons->twitter = 'horizontal';
			$buttons->facebook = 'none';
			$buttons->linkedin = 'right';
			$buttons->type = 'small';
		} else {
			// Small
			switch ($button) {
	...
	public function cleanUrl($url) {
		return preg_replace('/[?&]?hitcount=[0-9]$/i', '', $url);
	}

	public function matchVars($custom_vars) {
		$each_set = explode("\n", $custom_vars);
		$matched = false;
		if (count($each_set) > 0) {
			foreach ($each_set as $sets) {
				foreach (explode('|', $sets) as $values) {
					list($key, $value) = explode('=', $values, 2);
					$matched = (JRequest::getVar($key) == $value) ? true : false;
					if (!$matched) break;
				}
	...
	protected function contentCache() {
		$feed_url = $this->serviceUrl();
		if (!$feed_url) return false;
		jimport('joomla.filesystem.file');
		jimport( 'joomla.filesystem.folder');
		$path = JPATH_BASE.DS.'cache'.DS.'jsocial_comments_cache';
		$cache_file = $path.DS.'jsocial_comments_'.$this->_service.'_'.md5($feed_url).'.php';

		if (!JFile::exists($cache_file) || time() - filemtime($cache_file) > $this->_cache) {
			...
			$feed = '';
			$cache_rss = @file_get_contents($feed_url);
			if ($cache_rss) $feed .= $this->processContent($cache_rss);
			$die = '<?php die("Access Denied"); ?>#x#';
			$feed_content = @serialize($feed);
			$content = $die.$feed_content;
			JFile::write($cache_file, $content);
			return $feed;
		}
		return @unserialize(str_replace('<?php die("Access Denied"); ?>#x#', '', @JFile::read($cache_file)));
	}

	protected function serviceUrl() {
		if ($this->_service == 'facebook') return 'https://graph.facebook.com/comments/?ids='.urlencode($this->_url);
		return false;
	}
		...
		if ($this->_service == 'facebook') {
			...
			if (is_array($result)) {
				...
				if (isset($result['data']) || count($result['data']) > 0) {
					$result = $this->multiDimentional($result, 'message');
					$data = '';
	...
	protected function multiDimentional($array, $flag) {
		$flat = array();
		foreach ($array as $key => $value) {
			if (is_array($value)) {
				$flat = array_merge($flat, $this->multiDimentional($value, $flag));
			} else if ($key == $flag) {
				$flat[] = $value;
			}
		}
