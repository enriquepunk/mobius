...
<script type="text/javascript" src="scripts/galeria.js"></script>    
    ...
    <form action="contacto.php?sd" method="post">
    	<div id="formulario-lateral">
            ...
            <div class="campo-left">
            	<h3>E-mail</h3> <input type="text" required="required"  pattern="[\w-\.]{3,}@([\w-]{2,}\.)*([\w-]{2,}\.)[\w-]{2,4}" name="correo" value="<?php echo $_POST['correo']?>" />
            </div><!--FIn campo-left-->
            <div class="campo-left">
            	<h3>Telefono</h3> <input type="text" required="required" name="telefono" value="<?php echo $_POST['telefono']?>" />
            </div><!--FIn campo-left-->
        </div><!--Fin formulario-lateral-->
        <img src="images/ayudenos_mejorar.png" width="236" height="127" alt="Ayudenos a mejorar" style="margin-top:40px; float:right; margin-right:10px;" />
        <div id="formulario-bottom">
        	<div class="campo-bottom">
            	<h3>Servicio de Interes</h3>
                <input type="text" name="servicio" value="<?php echo $_POST['servicio']?>" />
            </div><!--Fin campo-bottom-->
            <div class="campo-check">
           		<div id="titulo-check">
            			<h3>¿Cómo nos conocio?</h3>
                </div><!---FIn titulo check-->
               <div class="texto-check"><h3>Por recomendación</h3></div> <input type="checkbox" name="referencias[]" value="Por recomendación" />
                <div class="texto-check"><h3>A través de dependencias gubernamentales</h3></div> <input type="checkbox" name="referencias[]" value="A través de dependencias gubernamentales" />
                <div class="texto-check"><h3>Internet y redes sociales</h3></div> <input type="checkbox" name="referencias[]" value="Internet y redes sociales" />
                <div class="texto-check"><h3>Cámaras empresariales</h3></div> <input type="checkbox" name="referencias[]" value="Cámaras empresariales" />
                <div class="texto-check"><h3>Otro</h3></div> <input type="checkbox" name="referencias[]" value="Otro" />
            </div><!--Fin campo-bottom-->
            <div class="comentarios">
            	<div id="titulo-cometarios">
            			<h3>Comentarios</h3>
                </div><!---FIn titulo check-->
               	<textarea name="comentario" style="width:367px; height:75px; float:right" value="<?php echo $_POST['comentario']?>"><?php echo $_POST['comentario']?></textarea>
                <input type="image" src="images/enviar.png" style="float:right; margin-top:20px;" class="btn-enviar"/>
            </div>
            <h6 style="font-size:12px;">*Es necesario rellenar todos los campos</h6>
	...
	if ( isset($_GET['sd']))
	{
		if(empty($_POST['Nombre']) || empty($_POST['Apellido']) || empty($_POST['correo']) || empty($_POST['telefono']) || empty($_POST['servicio']) || empty($_POST['referencias']) || empty($_POST['comentario']))
		{
			 echo "<div id='respuesta'>Alguno de los campos está vacios</div><br />";
		}
		else
		{
			// definimos el mensaje
			$mensaje="";
			$mensaje.="mensaje de contacto de financitconsultores.com:". "\n\n";
			$mensaje.="Nombre: ".$_POST['Nombre']."\n";
			$mensaje.="E-mail: ".$_POST['correo']."\n";
			$mensaje.="Telefono: ".$_POST['telefono']."\n";
			$mensaje.="Serivicio interesado: ".$_POST['servicio']."\n";
			$mensaje.="Fecha de cumpleaños: ".$_POST['dia']." de ".$_POST['mes']." \n\n";
			$mensaje.="Formas en las que conocieron financit consultores". "\n\n";
			$referencias =$_POST['referencias'];
			for( $j=0; $j<7; $j++ )
			{
				if(! empty($referencias[$j]))
				{
			...
			$cocobongo="mkt@cocobongo.com.mx";
			$email_destiny="eduardo@punkmkt.com, fernando.gomez@financitconsultores.com, francisco.gomez@financitconsultores.com";
			$subject="Nuevo contacto desde financitconsultores.com:  ";
			// verificamos si se envió
			if (mail($email_destiny,$subject,$mensaje,"From: Contact<".$_POST['correo'].">")) {
	    		echo "<div id='respuesta'>Gracias por tus comentarios!</div><br />";
			}
			else
			{
	    		echo "<div id='respuesta'>ErrorInesperado</div><br />";
			}
		}
	}
			?>
        ...
        </div><!--Fin formulario bottom--->
    </form>
    <img src="images/linea_contacto.png" width="559" height="1" alt="linea" style="margin:25px 0;" />
 </div><!--Fin Contenido-->
        <? include 'blog/gadgets.php'; ?>
