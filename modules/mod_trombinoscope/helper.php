...
class modTrombinoscopeHelper
{	
	protected static $lookup;
	
	static function getContacts($params, $module)
	{
		$params->def('selection', 'categories');
		$params->def('contact_id', '');
		$params->def('category', '');
		$params->def('includesubcategories', 'no');
		$params->def('cat_order', '');
		$params->def('order', 'cd.ordering');
		$params->def('count', '');
		$params->def('address_format', 'zss');		
		$params->def('field1', 'none');
		$params->def('field2', 'none');
		$params->def('field3', 'none');
		$params->def('field4', 'none');
		$params->def('field5', 'none');
		...
		if (isset($array_of_category_values['all']) && $array_of_category_values['all'] > 0) { // 'all' was selected
			...
			if ($get_sub_categories != 'no') {
				$categories_object = JCategories::getInstance('Contact');
				foreach ($categories_array as $category) {
					$category_object = $categories_object->get($category);
					if ($category_object->hasChildren()) {
		...
		$subquery = ' CASE WHEN ';
		$subquery .= $query->charLength('cd.alias');
		$subquery .= ' THEN ';
		$cd_id = $query->castAsChar('cd.id');
		$subquery .= $query->concatenate(array($cd_id, 'cd.alias'), ':');
		$subquery .= ' ELSE ';
		$subquery .= $cd_id.' END AS slug';
		...
		$subqueryfield1 = modTrombinoscopeHelper::getSubQuery($params->get('field1'), $params->get('address_format'), '1');
		$subqueryfield2 = modTrombinoscopeHelper::getSubQuery($params->get('field2'), $params->get('address_format'), '2');
		$subqueryfield3 = modTrombinoscopeHelper::getSubQuery($params->get('field3'), $params->get('address_format'), '3');
		$subqueryfield4 = modTrombinoscopeHelper::getSubQuery($params->get('field4'), $params->get('address_format'), '4');
		$subqueryfield5 = modTrombinoscopeHelper::getSubQuery($params->get('field5'), $params->get('address_format'), '5');
		
		//$query->select('cd.id, cd.catid, cd.name, cd.image,'.$subquery.','.$subquery2.','.$subquery3.','.$subquery4.$subqueryfield1.$subqueryfield2.$subqueryfield3.$subqueryfield4.$subqueryfield5);	
		$query->select('cd.id, cd.catid, trim(cd.name) as name, cc.lft as c_order, cd.image,'.$subquery.','.$subquery2.$subqueryfield1.$subqueryfield2.$subqueryfield3.$subqueryfield4.$subqueryfield5);
		
		$query->from('#__contact_details AS cd');
		$query->join('INNER', '#__categories AS cc ON cd.catid = cc.id');
		
		if ($selection == 'categories' && $categories !== null) {
			if ($categories != '') {
				$query->where('cd.catid IN ('.$categories.')');
			} // else we show all categories
		} else if ($selection == 'contact' && !empty($contact_id)) {
			$query->where('cd.id='.$contact_id);
		} else if ($selection == 'user' && $user->id > 0) {
		...
		$cat_order = $params->get('cat_order');
		$order = $params->get('order');
		switch ($order) {
			case 'cd.ordering ASC' :
			case 'cd.ordering DESC' :
			case 'cd.name ASC' :
			case 'cd.name DESC' : 
				if ($cat_order != '') {
					$query->order($cat_order.','.$order);
				} else {
					$query->order($order);
				}
				break;
		...
		foreach ($list as $item) {
			$item->firstpart = self::_substring_index($item->name, ' ', 1);
			$item->lastpart = self::_substring_index($item->name, ' ', -1);
		}

		if ($cat_order == 'cc.lft ASC') {
			switch ($order) {
				case 'lastpart ASC,firstpart ASC' : usort($list, "modTrombinoscopeHelper::sortCascLascFasc"); break;
				case 'lastpart DESC,firstpart DESC' : usort($list, "modTrombinoscopeHelper::sortCascLdescFdesc"); break;
				case 'firstpart ASC,lastpart ASC' : usort($list, "modTrombinoscopeHelper::sortCascFascLasc"); break;
				case 'firstpart DESC,lastpart DESC' : usort($list, "modTrombinoscopeHelper::sortCascFdescLdesc"); break;
				default: break;
			}
		} else if ($cat_order == 'cc.lft DESC') {
			switch ($order) {
				case 'lastpart ASC,firstpart ASC' : usort($list, "modTrombinoscopeHelper::sortCdescLascFasc"); break;
				case 'lastpart DESC,firstpart DESC' : usort($list, "modTrombinoscopeHelper::sortCdescLdescFdesc"); break;
				case 'firstpart ASC,lastpart ASC' : usort($list, "modTrombinoscopeHelper::sortCdescFascLasc"); break;
				case 'firstpart DESC,lastpart DESC' : usort($list, "modTrombinoscopeHelper::sortCdescFdescLdesc"); break;
				default: break;
			}
		} else {
			switch ($order) {
				case 'lastpart ASC,firstpart ASC' : usort($list, "modTrombinoscopeHelper::sortLascFasc"); break;
				case 'lastpart DESC,firstpart DESC' : usort($list, "modTrombinoscopeHelper::sortLdescFdesc"); break;
				case 'firstpart ASC,lastpart ASC' : usort($list, "modTrombinoscopeHelper::sortFascLasc"); break;
				case 'firstpart DESC,lastpart DESC' : usort($list, "modTrombinoscopeHelper::sortFdescLdesc"); break;
				default: break;
			}
		}
		
		if (intval($params->get('count')) > 0) {
			// get subset of the the data
			$list = array_slice($list, 0, intval($params->get('count')), true);
		}
	...
	/* sort by by category ASC firstpart DESC lastpart DESC */
	...
	static function getSubQuery($field, $format, $index)
	{		
		$subquery = '';
		...
		if ($field != "none") {
			...
			if ($field == "empty") {
				...
				switch ($format) {
					case 'ssz' :
						$subqueryformattedaddress = "CONCAT(cd.suburb, ', ', cd.state, ' ', cd.postcode)";
						break;
					case 'zss' :
						$subqueryformattedaddress = "CONCAT(cd.postcode, ' ', cd.suburb, ', ', cd.state)";
						break;
					case 'zs' :
						$subqueryformattedaddress = "CONCAT(cd.postcode, ' ', cd.suburb)";
						break;
					case 'sz' :
						$subqueryformattedaddress = "CONCAT(cd.suburb, ' ', cd.postcode)";
						break;
					case 'ss' :
						$subqueryformattedaddress = "CONCAT(cd.suburb, ', ', cd.state)";
						break;						
					default :
						$subqueryformattedaddress = '';
					break;
				...
				$subquery .= $field." AS field".$index.", '".$field."' AS fieldname".$index;
			}
		}
	...
	static function getFieldOutput($field, $fieldname, $keepspace, $index) 
	{		
		$html = '';
				
		// field is empty (null) if 'none' is selected or no information has been found in the database, except for formatted address...
		
		if ($fieldname == "formattedaddress") { // trim the field if it contains nothing 
			$elements = preg_split ('/[\s]*[,][\s]*/', $field); // deals with "elt1  ,elt2,   elt3  "
			...
			// to avoid commas or spaces if missing info
			if ($i == 0) {
				$field = '';
			} else if ($i == 1) {
				$field = $unique_element;
			}	
		}
		
		if (!empty($field)) {
			$html .= '<div class="personfield'.$index.'">';
			
			switch ($fieldname) {
				case 'cd.con_position' :
					$html .= '    <span class="personposition">'.$field.'</span>';
					break;
				case 'cd.telephone' :
					$html .= '    <span class="persontel">'.$field.'</span>';
					break;
				case 'cd.mobile' :
					$html .= '    <span class="personmobile">'.$field.'</span>';
					break;
				case 'cd.fax' :
					$html .= '    <span class="personfax">'.$field.'</span>';
					break;
				case 'cd.email_to' :
					...
					break;
				case 'cd.webpage' :
					$html .= '    <a class="personwebpage" href="'.$field.'" target="_blank" title="'.$field.'">';
					$html .= '        <span>'.$field.'</span>';
					$html .= '    </a>';
					break;
				case 'cd.address' :
					$html .= '    <span class="personaddress">'.nl2br($field).'</span>';
					break;
				case 'cd.suburb' :
					$html .= '    <span class="personsuburb">'.$field.'</span>';
					break;
				case 'cd.state' :
					$html .= '    <span class="personstate">'.$field.'</span>';
					break;
				case 'cd.country' :
					$html .= '    <span class="personcountry">'.$field.'</span>';
					break;
				case 'cd.misc' :
					$html .= '    <div class="personmiscinfo">'.$field.'</div>';
					break;
				case 'formattedaddress' :
					$html .= '    <span class="personformattedaddress">'.$field.'</span>';
					break;
				default :
					$html .= '    <span class="_empty">&nbsp;</span>'; // 'empty' field
					break;
			}
			...
			switch ($fieldname) {
				case 'cd.con_position' :
					$html .= '    <span class="personposition_empty">&nbsp;</span>';
					break;
				case 'cd.telephone' :
					$html .= '    <span class="persontel_empty">&nbsp;</span>';
					break;
				case 'cd.mobile' :
					$html .= '    <span class="personmobile_empty">&nbsp;</span>';
					break;
				case 'cd.fax' :
					$html .= '    <span class="personfax_empty">&nbsp;</span>';
					break;
				case 'cd.email_to' :
					$html .= '    <span class="personemail_empty">&nbsp;</span>';
					break;
				case 'cd.webpage' :
					$html .= '    <span class="personwebpage_empty">&nbsp;</span>';
					break;
				case 'cd.address' :
					$html .= '    <span class="personaddress_empty">&nbsp;</span>';
					break;
				case 'cd.suburb' :
					$html .= '    <span class="personsuburb_empty">&nbsp;</span>';
					break;
				case 'cd.state' :
					$html .= '    <span class="personstate_empty">&nbsp;</span>';
					break;
				case 'cd.country' :
					$html .= '    <span class="personcountry_empty">&nbsp;</span>';
					break;
				case 'cd.misc' :
					$html .= '    <div class="personmiscinfo_empty">&nbsp;</div>';
					break;
				case 'formattedaddress' :
					$html .= '    <span class="personformattedaddress_empty">&nbsp;</span>';
					break;				
				default :
					break;
			}	
	...
	*/
	public static function getContactRoute($id, $catid, $extraoptions = '')
	{
		$needles = array(
			'contact'  => array((int) $id)
		);

		$link = 'index.php?option=com_contact&view=contact&id='.$id.$extraoptions;
		if ($catid > 1) {
