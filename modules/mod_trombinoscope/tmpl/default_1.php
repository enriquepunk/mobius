...
require_once(JPATH_SITE.'/modules/mod_trombinoscope/helper.php');

$i = 0;
$oddoreven = "even";
?>

<div class="trombinoscope_<?php echo $class_suffix; ?> personlist">
<?php foreach ($list as $item) : ?>	
	
	<?php	
		// link
		//$link = JRoute::_('index.php?option=com_contact&view=contact&id='.$item->slug.'&catid='.$item->catslug.'&Itemid='.$params->get('menulink'));
		//$link = JRoute::_(ContactHelperRoute::getContactRoute($item->slug, $item->catid));
			
		$link = '';
		$extra_attributes = '';	
		switch ($show_link) {
			case 'standard' :
				$link = htmlspecialchars(modTrombinoscopeHelper::getContactRoute($item->slug, $item->catslug));
				break;
			case 'popup' : // open in a modal window
				$link = htmlspecialchars(modTrombinoscopeHelper::getContactRoute($item->slug, $item->catslug, '&tmpl=component'));
		...
		if ($format_style == "lastspacefirst") {
			if (!empty($item->lastpart)) {
				$formatted_name = $item->lastpart." ".$item->firstpart;
			} else {
				$formatted_name = $item->firstpart;
			}
		} else if ($format_style == "firstspacelast") {
			if (!empty($item->firstpart)) {
				$formatted_name = $item->firstpart." ".$item->lastpart;
			} else {
				$formatted_name = $item->lastpart;
			}
		} else if ($format_style == "lastcomafirst") {
			if (!empty($item->lastpart)) {
				$formatted_name = $item->lastpart.", ".$item->firstpart;
			} else {
				$formatted_name = $item->firstpart;
			}
		} else if ($format_style == "firstcomalast") {
			if (!empty($item->firstpart)) {
				$formatted_name = $item->firstpart.", ".$item->lastpart;
			} else {
				$formatted_name = $item->lastpart;
		...
		// picture positioning
		if ($params->get('photo_align') == "alternate") {
			if ($i % 2) {
				$photo_align = "left";
			} else {
				$photo_align = "right";
...
<!--	<div class="person">
		<div class="innerperson <?php echo $oddoreven; ?>">
			<div class="personpicture" style="float: <?php echo $photo_align ?>">
				<?php if ($show_link) : ?>			
					<a <?php echo $extra_attributes; ?> href="<?php echo $link; ?>" title="<?php echo $formatted_name; ?>" >
						<?php if (!empty($item->image)) : ?>			
							<?php echo JHTML::_('image', $item->image, $formatted_name, array('class' => 'personpictureimg')); ?>
						<?php elseif (!empty($default_picture)) : ?>
							<?php echo JHTML::_('image', $default_picture, $formatted_name, array('class' => 'personpictureimg')); ?>
						<?php else : ?>
			...
			<div class="personinfo <?php echo $photo_align ?>">				
				<?php if (empty($link_text) && $show_link) : ?>
					<div class="personname">
						<a <?php echo $extra_attributes; ?> href="<?php echo $link; ?>" title="<?php echo $formatted_name; ?>" >
							<span><?php echo $formatted_name; ?></span>
						</a>
					</div>
				<?php else : ?>
					<div class="personname"><?php echo $formatted_name; ?></div>
				<?php endif; ?>				
				
				<?php 
					if (!empty($item->fieldname1)) { echo modTrombinoscopeHelper::getFieldOutput($item->field1, $item->fieldname1, $keep_space, 1);	}			
					if (!empty($item->fieldname2)) { echo modTrombinoscopeHelper::getFieldOutput($item->field2, $item->fieldname2, $keep_space, 2);	}			
					if (!empty($item->fieldname3)) { echo modTrombinoscopeHelper::getFieldOutput($item->field3, $item->fieldname3, $keep_space, 3);	}			
					if (!empty($item->fieldname4)) { echo modTrombinoscopeHelper::getFieldOutput($item->field4, $item->fieldname4, $keep_space, 4);	}			
					if (!empty($item->fieldname5)) { echo modTrombinoscopeHelper::getFieldOutput($item->field5, $item->fieldname5, $keep_space, 5); }
				?>	
					
				<?php if (!empty($link_text) && $show_link) : ?>					
					<div class="personlink">
						<a <?php echo $extra_attributes; ?> href="<?php echo $link; ?>" title="<?php echo $link_text; ?>" >
...
<div class="trombinoscope_<?php echo $class_suffix; ?> personlist clearfix" >
    
  
    <?php $jj=1;foreach ($list as $item) : ?>	
	...
	?>
    <?php   if($jj % 5 == 1) {echo '<div class="item-group">';}?>
      <div class="item-horario">
            <table align="center">
                <tr>
                    <td width="25%" style="padding: 10px 30px 9px 85px; border-bottom: 3px solid #55971d;border-right: 3px solid white;">
  ...
  <?php if($jj % 5 == 0) echo ' </div>';?>

<?php $jj++; endforeach;  if($jj % 5 != 1) echo ' </div>';?>
